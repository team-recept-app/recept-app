...
/* modul-szintű shaderek – nincs exhaustive-deps warning */
const VERT = /* glsl */ `
...
const FRAG = /* glsl */ `
  ...
  const vec3 baseColor1=vec3(0.85,0.35,1.0);
  const vec3 baseColor2=vec3(0.35,0.85,1.0);
  const vec3 baseColor3=vec3(0.07,0.09,0.65);
...
export default function Orb({
  hue = 200,                // kicsit kékes-lila
  hoverIntensity = 0.7,     // látványosabb
  rotateOnHover = true,
  forceHoverState = true    // autoplay, hogy mindig „éljen”
}: OrbProps) {
  ...
  useEffect(() => {
    ...
    // innentől LOKÁLIS, nem-null változóval dolgozunk → nincs "possibly null"
    const container = el;
